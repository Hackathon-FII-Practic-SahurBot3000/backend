...
public interface HackathonTeamRepository extends JpaRepository<HackathonTeam, Long> {
    @Query("SELECT CASE WHEN COUNT(ht) > 0 THEN true ELSE false END FROM HackathonTeam ht JOIN ht.team.teamMembers tm WHERE ht.hackathon.id = :hackathonId AND tm.user.id = :userId")
