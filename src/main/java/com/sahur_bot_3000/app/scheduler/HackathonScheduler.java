...
public void updateHackathonStates() {
    ...
    for (Hackathon h : hackathons) {
        if (h.getHackathonState() == HackathonState.Pending && now.after(h.getPendingAt())) {
            h.setHackathonState(HackathonState.Ongoing);
        } else if (h.getHackathonState() == HackathonState.Ongoing && now.after(h.getVotingAt())) {
            h.setHackathonState(HackathonState.Voting);
        } else if (h.getHackathonState() == HackathonState.Voting && now.after(h.getEndedAt())) {
            h.setHackathonState(HackathonState.Closed);
        }
    ...
    public void generateDailyHackathons() {
        for (HackathonType type : HackathonType.values()) {
            ...
            Date now = new Date();
            Date end = new Date(now.getTime() + 48 * 60 * 60 * 1000);
